...
class InterceptRelativeLayout : RelativeLayout {
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        if (!this.isInEditMode) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.InterceptRelativeLayout)
            isIntercept = ta.getBoolean(R.styleable.InterceptRelativeLayout_isIntercept, false)
            ta.recycle()
        }
    }
