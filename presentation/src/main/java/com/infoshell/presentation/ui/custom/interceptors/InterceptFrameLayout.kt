...
class InterceptFrameLayout : FrameLayout {
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        if (!this.isInEditMode) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.InterceptFrameLayout)
            isIntercept = ta.getBoolean(R.styleable.InterceptFrameLayout_isIntercept, false)
            ta.recycle()
        }
    }
